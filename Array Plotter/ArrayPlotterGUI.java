    ...
    implements GridChangeListener
{
    ...
    // in milliseconds.
    private static final int LONGEST_DELAY_MSECS = 100;  // slowest
    private static final int SHORTEST_DELAY_MSECS = 1;  // fastest


    private GridPlotterGUI display;
    private boolean colorArrayCreated;
    private boolean[][] previousCA;
    ...
    public void update(boolean[][] ca)
    {
        ...
        if (Arrays.deepEquals(ca, previousCA))
        {
            //          JOptionPane.showMessageDialog(
            //              null,
            //              "No colorArray elements have changed value since last update." +
            //              "\nOnly X-Fill should get this warning message.",
            //              "Warning",
        ...
        {
            if (previousCA == null ||
                 ca.length != previousCA.length ||
                 ca[0].length != previousCA[0].length)
                previousCA = new boolean[ca.length][ca[0].length];
            for (int r = 0; r < ca.length; r++)
                for (int c = 0; c < ca[0].length; c++)
                    previousCA[r][c] = ca[r][c];
        }
        display.sleep();
    }
